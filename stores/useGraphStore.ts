/* =========================================================
   stores/useGraphStore.ts — graph state, undo/redo, persistence
   ========================================================= */
...
export const useGraphStore = defineStore('graph', () => {
  ...
  function addChild(parentId: string, label = 'new idea'): string | null {
    ...
    for (let deg = 0; deg < 360; deg += 12) {
      ...
      let dmin = Infinity
      for (const s of refs) { const d2 = (s.x - tx) ** 2 + (s.y - ty) ** 2; if (d2 < dmin) dmin = d2 }
  ...
  function deleteSubtree(id: string) {
    const root = rootNode(); if (!id || id === root?.id) return
    pushHistory()
    ...
    if (toRemove.has(selectedId.value ?? '')) selectedId.value = root?.id ?? null
  }
  ...
  function moveNode(id: string, x: number, y: number) {
    const n = nodeById(id); if (!n) return; n.x = x; n.y = y
  }
  ...
  function reset() {
    ...
    title.value = s.title; nextId.value = s.nextId
    nodes.value.splice(0, nodes.value.length, ...s.nodes)
    selectedId.value = rootNode()?.id ?? null; editingId.value = null; linkFromId.value = null
  ...
  function importJSON(json: string): { ok: boolean; error?: string } {
    try {
      ...
      editingId.value = null; linkFromId.value = null
      return { ok: true }
  ...
  function applyLayout(positions: { id: string; x: number; y: number }[]) {
    ...
    pushHistory()
    nodes.value = nodes.value.map(n => { const pos = posMap.get(n.id); return pos ? { ...n, x: pos.x, y: pos.y } : n })
  }
  ...
  return {
    ...
    isLayouting: skipHydrate(isLayouting),
    canUndo, canRedo, undo, redo,
    nodeById, rootNode, childrenOf, ancestorsOf, levelOf, descendantsOf,
    addNodeAt, addChild, deleteSubtree, setLabel, moveNode, beginDrag, endDrag,
    reparent, setTitle, reset, exportJSON, importJSON, pushHistory,
